...
public class UrlDaoImpl {
	...
	public Url findByUrl(String urlName) throws Exception {
		...
		try {
			...
			String sql = "SELECT  url.url_id, url.role_id, r.rolename, url.module_id, url.env_id, m.module_name, "
					...
					+ " FROM URL url "
					+ "left outer join MODULE m on url.module_id = m.module_id "
					+ "left outer join ENVIRONMENT_MASTER em on url.env_id = em.env_id "
	...
	public List<Url> findAll(boolean isGuestUser) {
		...
		if(isGuestUser) {
			sql = sql + " where url.visible = 'true'";
		}
	...
	public void createUrl(Url url) throws Exception {
		try {
			if (url != null && url.getUrlId() == 0) {
				...
				String sql = "INSERT INTO URL (name, description,url,username,password,module_id, env_id, role_id,visible,email, create_user, create_date) "
						+ " VALUES (:name, :description, :url, :username, :password, :module_id,:env_id, :role_id, :visible, :email, :create_user, :create_date);";

				Map<String, Object> parameters = new HashMap<String, Object>();
				parameters.put("name", url.getName());
				parameters.put("description", url.getDescription());
				parameters.put("url", url.getUrl().trim());
				parameters.put("username", url.getUsername());
				parameters.put("password", passwordEncodeDecode.base64encode(url.getPassword()));
				parameters.put("module_id", url.getModuleId());
				parameters.put("env_id", url.getEnvId());
		...
		try {
			if (url != null && url.getUrlId() > 0) {
				...
				String sql = "update URL set name=:name, description=:description, url=:url,"
						+ "username=:username,password=:password,module_id=:module_id, env_id=:env_id,role_id=:role_id,visible=:visible,email=:email,"
				...
				log.debug(url.getUrl() + " URL not updated");
			}
		} catch (Exception e) {
	...
	/**
	 * This method deletes an URL
	 * @param url
	 * @throws Exception
	 */
	public void deleteUrl(Url url) throws Exception {
		try {
			if (url != null && url.getUrlId() > 0) {
				log.debug("Going to delete " + url.getName());
				String sql = "delete from URL where url_id=:url_id";
				...
				log.debug(url.getUrl() + " URL not deleted");
			}
		} catch (Exception e) {
			log.error("Unable to delete URL for url_name: " + url.getName(), e);
			throw e;
		}
	}
	...
	private static final class UrlMapper implements RowMapper<Url> {
		...
		public Url mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			if(rs.getInt("module_id") > 0) url.setModuleId(rs.getInt("module_id"));
			if(rs.getInt("env_id") > 0) url.setEnvId(rs.getInt("env_id"));
