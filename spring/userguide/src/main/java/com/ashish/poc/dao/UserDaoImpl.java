...
public class UserDaoImpl {
	...
	public Users findByUserName(String name) throws Exception {
		...
		try {
			...
			String sql = "SELECT u.*,'' as rolename FROM users u WHERE lower(u.username)=:username";
	...
	public Users findByUserId(int userId) throws Exception {
		...
		try {
			...
			String sql = "SELECT u.*,'' as rolename FROM users u WHERE u.user_id=:userId";
	...
	public List<Users> findAll() {
		...
		String sql = "SELECT u.*, r.rolename FROM users u left outer join role r on u.role_id=r.role_id";
...
//				String sql = "update USERS set name='a' where user_id=1";
	...
	private static final class UserMapper implements RowMapper<Users> {
		...
		public Users mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			PasswordEncodeDecodeUtil passwordEncodeDecode = new PasswordEncodeDecodeUtil(); 
			Users user = new Users();
			user.setUserId(rs.getInt("user_id"));
			user.setUsername(rs.getString("username"));
			user.setName(rs.getString("name"));
			user.setPassword(passwordEncodeDecode.base64decode(rs.getString("password")));
