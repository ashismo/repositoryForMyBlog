...
public class UserGuideWSImpl {
	...
	public boolean isGeneralUser() throws Exception {
		return true;
	}
...
//			commonUtil.backupCurrentDatabaseState();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@WebMethod
	public UserDataModel deleteUrl(UserDataModel userDataModel)
			throws Exception {
		try {
			urlService.deleteUrl(userDataModel);
		} catch (Exception e) {
			log.error("Unable to delete URL", e);
			userDataModel.setErrorMsg("Unable to delete URL");
		}
		return userDataModel;
	}
...
//	@PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_GENERAL')")
	@WebMethod
	public String accessDenied(@Context HttpHeadersImpl headers) throws Exception {
		UserDataModel userDataModel = new UserDataModel();
