...
@WebAppConfiguration
public class RetailBranchFyCloseWSTest {
	private static final Logger logger = Logger.getLogger(RetailBranchFyCloseWSTest.class);
	
	private MockMvc mockMvc;
	@Autowired
	private WebApplicationContext wac;
	
	private String closeBranchFyJson = null;
	...
	public void runBefore() {
		try {
			...
			om.setDateFormat(df);
			closeBranchFyJson = JunitTestUtil.getFileContent("inputJson/retail/branch/closeBranchFy.json");
		} catch (Exception e) {
	...
	@Test
	public void testBranchFYClose() {
		closeBranchFy();
	}

	
	private void closeBranchFy() {
		try {
			MvcResult result = this.mockMvc.perform(post("/rest/createRetailBranch")
				 ...
				 .content(closeBranchFyJson)
				).andExpect(status().isOk())
				.andExpect(content().contentType("application/json"))
				.andReturn();
			
			UIModel uiModel = getUIModel(result, "outputJson/retail/branch/closeBranchFy.json");
			
			assertNull(uiModel.getErrorMsg());
		} catch(Exception e) {
			logger.error("Error while closing FY for a branch", e);
			Assert.fail("Error while closing FY for a branch");
		}
	}
