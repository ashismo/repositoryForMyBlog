...
public class RetailTransactionManagementWSTest {
	...
	public void runBefore() {
		try {
			...
			om.setDateFormat(df);
			addStockEntryJson = JunitTestUtil.getFileContent("inputJson/retail/branch/transaction/saveStockTransaction.json");
		} catch (Exception e) {
	...
	private void saveStockTransaction() {
		try {
			...
			UIModel uiModel = getUIModel(result, "outputJson/retail/branch/transaction/addStockEntry.json");
			assertNull(uiModel.getErrorMsg());
		} catch(Exception e) {
			logger.error("Error while saving stock transaction", e);
			Assert.fail("Error while adding stock transaction");
		}
	}
	
	
	private void getStockTransactionForATranId() {
		try {
			MvcResult result = this.mockMvc.perform(get("/rest/getStockTransaction")
					 .contentType("application/json").header("Authorization", "Basic " + Base64.getEncoder().encodeToString("ashish:ashish".getBytes()))
					).andExpect(status().isOk())
					.andExpect(content().contentType("application/json"))
					.andReturn();
				
			UIModel uiModel = getUIModel(result, "outputJson/retail/branch/transaction/getAllStocksForABranch.json");
			assertNull(uiModel.getErrorMsg());
		} catch(Exception e) {
