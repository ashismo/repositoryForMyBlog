...
@WebAppConfiguration
public class RetailDocumentWSTest {
	private static final Logger logger = Logger.getLogger(RetailDocumentWSTest.class);
	
	private MockMvc mockMvc;
	@Autowired
	private WebApplicationContext wac;
	
	private String uploadDocumentJson = null;
	...
	public void runBefore() throws Exception {
		try {
			...
			om.setDateFormat(df);
			uploadDocumentJson = JunitTestUtil.getFileContent("inputJson/retail/branch/uploadDocument.json");
	...
	@Test
	public void testRetailDocument() {
		uploadDocument();
//		getBranch();
	}
	...
	private void uploadDocument() {
		try {
			assertNotNull(uploadDocumentJson);
			MvcResult result = this.mockMvc.perform(fileUpload(new URI("/rest/document")).file("file", uploadDocumentJson.getBytes())
					.param("uiModel", uploadDocumentJson)
				 .contentType(MediaType.MULTIPART_FORM_DATA)
				 .accept(MediaType.APPLICATION_JSON)
				 .header("Authorization", "Basic " + Base64.getEncoder().encodeToString("ashish:ashish".getBytes()))
				).andExpect(status().isOk())
				.andExpect(content().contentType("application/json"))
				.andReturn();
			
			UIModel uiModel = getUIModel(result, "outputJson/retail/branch/uploadDocument.json");
