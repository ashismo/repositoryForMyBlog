...
.directive('wmwindow', function () {
	return {
      ...
      data : '=',
        ...
        var app = angular.copy(scope.$root.miniAppNonMatch);
        var data = angular.copy(scope.$parent.article);
        var index = scope.$root.miniAppNonMatch.indexOf(scope.$parent.article);
        if(index !== -1){
          scope.$root.miniAppNonMatch.splice(index,1);
        }
        console.log(scope);
